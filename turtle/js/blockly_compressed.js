...
Blockly.Trashcan.prototype.createDom = function() {
	...
	this.svgBody_.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href",this.BODY_URL_);
	...
	this.svgLid_.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", this.LID_URL_);
    return this.svgGroup_
};
...
Blockly.Xml.domToBlock_ = function(a, b) {
    ...
    if (!c) throw "Block type unspecified: \n" + b.outerHTML;
    var d = new Blockly.Block(a, c);
    d.initSvg();
    var e = b.getAttribute("inline");
    e && d.setInputsInline("true" == e);
    (e = b.getAttribute("collapsed")) && d.setCollapsed("true" == e);
    (e = b.getAttribute("disabled")) && d.setDisabled("true" == e);
    (e = b.getAttribute("deletable")) && d.setDeletable("true" == e);
    (e = b.getAttribute("movable")) && d.setMovable("true" == e);
    (e = b.getAttribute("editable")) && d.setEditable("true" ==
        e);
    for (var f = null, e = 0, g; g = b.childNodes[e]; e++)
        if (3 != g.nodeType || !g.data.match(/^\s*$/)) {
            for (var f = null, h = 0, k; k = g.childNodes[h]; h++) 3 == k.nodeType && k.data.match(/^\s*$/) || (f = k);
            h = g.getAttribute("name");
            switch (g.nodeName.toLowerCase()) {
                case "mutation":
                    d.domToMutation && d.domToMutation(g);
                    break;
                case "comment":
                    d.setCommentText(g.textContent);
                    (f = g.getAttribute("pinned")) && d.comment.setVisible("true" == f);
                    f = parseInt(g.getAttribute("w"), 10);
                    g = parseInt(g.getAttribute("h"), 10);
                    isNaN(f) || isNaN(g) || d.comment.setBubbleSize(f,
                        g);
                    break;
                case "title":
                    d.setTitleValue(g.textContent, h);
                    break;
                case "value":
                case "statement":
                    g = d.getInput(h);
                    if (!g) throw "Input " + h + " does not exist in block " + c;
                    if (f && "block" == f.nodeName.toLowerCase())
                        if (f = Blockly.Xml.domToBlock_(a, f), f.outputConnection) g.connection.connect(f.outputConnection);
                ...
                case "next":
                    if (f && "block" == f.nodeName.toLowerCase()) {
                        ...
                        f = Blockly.Xml.domToBlock_(a, f);
                        if (!f.previousConnection) throw "Next block does not have previous statement.";
                        d.nextConnection.connect(f.previousConnection)
                    }
            }
        }(c = d.nextConnection && d.nextConnection.targetBlock()) ? c.render(): d.render();
    return d
};
...
Blockly.parseOptions_ = function(a) {
    ...
    if (b) var c = !1,
        ...
        e = !1,
        f = null;
    else(f = a.toolbox) ? ("string" != typeof f && "undefined" == typeof XSLTProcessor && (f = f.outerHTML), "string" == typeof f && (f = Blockly.Xml.textToDom(f)), c = !!f.getElementsByTagName("category").length) : (f = null, c = !1), d = a.trashcan, void 0 === d && (d = c), e = a.collapse, void 0 === e && (e = c);
    if (f && !c) var g = !1;
